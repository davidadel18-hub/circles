...
export default function PostDetails() {
   ...
   console.log(id);
    ...
    function getPostDetails(){
       return axios.get(`https://route-posts.routemisr.com/posts/${id}` , {headers:{Authorization:`Bearer ${localStorage.getItem('token')}`}})
    }
   ...
   return (
     <div className="flex justify-center items-center min-h-[calc(100vh-4rem)] bg-[#1A0B2E]" dir="ltr">
       <div className="inline-flex items-center gap-2.5 bg-[#1A0B2E] border border-[#00F2FE]/30 shadow-[0_0_15px_rgba(0,242,254,0.1)] font-semibold rounded-2xl text-sm px-5 py-3 text-[#00F2FE]">
         {/* أيقونة السبينر المتحركة بالألوان الجديدة */}
         <svg 
           aria-hidden="true" 
         ...
         {/* نص التحميل المودرن بالألوان الجديدة */}
 ...
 if(isError){
 return(<> <div className="flex justify-center items-center min-h-[calc(100vh-4rem)] bg-[#1A0B2E]" dir="ltr">
  ...
  return (
    <>
      <div className=" w-full bg-[#1A0B2E] text-[#FFFFFF] min-h-[calc(100vh-4rem)]">
          <div className="container mx-auto p-4 md:p-10">
            <div className=" grid grid-cols-1 gap-6 max-w-2xl mx-auto">
          <PostCard post={data?.data.data.post} manyComments/>
            </div>
          </div>
        </div>
    </>
